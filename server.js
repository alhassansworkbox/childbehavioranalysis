/* ============================================================
   ...
   تحليل صحة وسلوك طفلك — Backend server (flat / single-folder)
   Serves index.html + admin.html and proxies to the Notion API.
   ============================================================ */
...
app.get('/api/config', (_req, res) => res.json({ whatsapp: WHATSAPP_NUMBER }));
...
app.post('/api/submit', async (req, res) => {
  ...
  try {
    ...
    if (!r.ok) {
      console.error('Notion create page failed:', r.status, await r.text());
...
app.get('/api/admin/stats', async (req, res) => {
  ...
  try {
    ...
    const concernCounts = {};
    props.forEach((p) => { const c = readSelect(p['Main Concern']); if (c) concernCounts[c] = (concernCounts[c] || 0) + 1; });
    const topConcern = Object.entries(concernCounts).sort((a, b) => b[1] - a[1])[0] || [null, 0];

    const statusCounts = {};
    props.forEach((p) => { const s = readSelect(p['Health Status']); if (s) statusCounts[s] = (statusCounts[s] || 0) + 1; });

    const latest = rows.slice(0, 12).map((r) => {
      const p = r.properties || {};
      return {
        name: readTitle(p['Child Name']), phone: readPhone(p['Parent Phone Number']),
        age: readSelect(p['Child Age']), concern: readSelect(p['Main Concern']),
        total: readNumber(p['Total Score']), status: readSelect(p['Health Status']),
        leadStatus: readSelect(p['Lead Status']), highRisk: readCheckbox(p['High Risk Flag']),
        date: r.created_time,
      };
    });
    ...
    res.json({
      ...
      concernCounts, statusCounts,
      highRiskCount: props.filter((p) => readCheckbox(p['High Risk Flag'])).length,
      followUpCount: props.filter((p) => readCheckbox(p['Needs Follow Up'])).length,
      latest,
    });
  } catch (err) {
...
app.listen(PORT, () => {
  ...
  if (!NOTION_API_KEY) console.log('    ⚠️  NOTION_API_KEY not set — submissions will fail until you add it.\n');
});
